            ...
            file.exists(`${rw_pos}`, function (exists) {
                if (exists) {
                    ...
                    shell.shellFunc(sh_order).then(function (result) {
                        file.writeFile(`${rw_pos}${f_name}.txt`, buffer.toString('utf8'), function (err, w_res) {
                            console.log(`finished ${rw_pos} ${page_link.city} ${page_link.page}`)
                            if(err) throw(err)
                            cb('finished')
                        })
...
var analyseAddrCity = function(addr, cn_city){
    var all_citys = web_src.qgcitys
    for(var aci in all_citys){
        var i_city = all_citys[aci].cn.split('市')[0]
        if(i_city == all_citys[aci].cn)
            i_city = all_citys[aci].cn.split('省')[0]
        var h_city = `^${i_city}`, mhead = new RegExp(h_city, "g")
        if(addr.match(h_city))
            return addr
    }
    var fr_city = cn_city.split('市')[0], head_str = `^${fr_city}`
    if(fr_city == cn_city)
        fr_city = cn_city.split('省')[0]
    var revar = new RegExp(head_str, "g")
    if(!addr.match(revar))
        addr = cn_city+addr
    else{
        if(!addr.match('市') || !addr.match('省'))
            addr = cn_city+addr.split(fr_city)[1]
    }
    return addr
}
var loopForFetch = function(f_city, f_page, rw_dir, s_site){
    ...
    nestimeout = setTimeout(function () {
...
var startLoopFetch = function(dur){
    ...
    randomFetch(0)
    randomFetch(1)
    var curLoopEnd = fetchInterval(dur)
...
var fastSrcAddr = function(f_file, c_local, scope, city, cb){
    if(delayed){
        delayed = false
        return cb(null)
    }
    ...
    if(info.waddrs.length){
        var recurs_scope = function(addrs){
            ...
            var re_addr = analyseAddrCity(addrs[0], city)
            reanynisAddrForClient(re_addr).then(function(local){
...
var find_files = function(destination, j_finding, site, f_local, c_scope, gate, cn_city, cb){
    ...
    if(t_file) {
        fastSrcAddr(t_file, f_local, c_scope, cn_city, function (f_res) {
            if (f_res && !fetched) {
...
module.exports = {
    webIndexesFetch: function(duration){
